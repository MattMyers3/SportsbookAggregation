    ...
    public class FoxBetSportsBook : ISportsBook
    {
        ...
        private static IEnumerable<GameOffering> GetFootballOfferings(dynamic initialJson)
        {
            var usaCategoryJson = ((IEnumerable) initialJson.categories).Cast<dynamic>()
                .First(g => g.name == "USA");
            var results = GetNflGameOfferings(usaCategoryJson);

            return GetNflGameOfferings(usaCategoryJson);
        }

        private static IEnumerable<GameOffering> GetNflGameOfferings(dynamic usaCategoryJson)
        {
            var nflCompetitionJson =
                ((IEnumerable) usaCategoryJson.competition).Cast<dynamic>().FirstOrDefault(g => g.name == "NFL");
            ...
            {
                gameOfferings.Add(ParseNflGameOffering(game));
            }
